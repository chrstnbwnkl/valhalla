...
//   w.add_trackpoint(...);
//   w.add_course_point({.lat = 50.94, .lon = 6.96, .timestamp = ..., .name = "turn", .type = fit::CoursePointType::left});
//   std::string fit_data = w.to_string();
//
// usage (file):
...
public:
    ...
    explicit CourseWriter(const std::string& filename)
        : filename_(filename) {}

    void set_course_name(const std::string& name) { course_name_ = name; }
    void set_sport(Sport sport) { sport_ = sport; }
    ...
    size_t num_trackpoints() const { return trackpoints_.size(); }
    size_t num_course_points() const { return course_points_.size(); }
...
private:
    ...
    void write_definition(std::ostream& out, uint8_t local_msg, uint16_t global_msg,
    ...
    void write_course(std::ostream& out) {
        uint8_t name_len = std::max<uint8_t>(16, static_cast<uint8_t>(
            std::min<size_t>(course_name_.size() + 1, 32)));
    ...
    void write_course_point_definition(std::ostream& out) {
        write_definition(out, 5, mesg::course_point, {
            {253, 4, base_type::uint32},  // timestamp
            {1,   4, base_type::sint32},  // position_lat
            {2,   4, base_type::sint32},  // position_long
            {3,   4, base_type::uint32},  // distance
            {4,   1, base_type::enum8},   // type
            {5,  16, base_type::string},  // name
        });
    }
